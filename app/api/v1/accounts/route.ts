...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `https://${servicesHost}/api/${API_VERSION}/accounts`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: `Failed accounts request: ${response.status} ${text}` },
    ...
    return NextResponse.json(
      { error: `Failed accounts request: ${error?.message || 'Unknown error'}` },
      { status: 500 }
    );
  }
}
