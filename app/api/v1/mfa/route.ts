...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `https://${servicesHost}/api/${API_VERSION}/mfa`;
    ...
    if (!response.ok) {
      return NextResponse.json({ error: `Failed MFA request: ${response.status} ${text}` }, { status: response.status });
    }
    ...
    return NextResponse.json(
      { error: `Failed MFA request: ${error?.message || 'Unknown error'}` },
      { status: 500 }
    );
  }
}
